...
public class spring开发笔记 {
    ...
    /**
     ...
     2.运行期生成。
        ...
        具体实现可以分为两种：
            继承：
                动态生成一个类的子类，在子类里面对父类进行增强。在客户端获取父类的时候，返回的时子类。这要求编写的程序必须面向父类编程，这样替换成子类，同样可以运行。
            实现：
                动态生成一个接口的实现，这样求编写的程序必须面向接口编程，这样使用代理类，同样可以使用。
     ...
     多实现了一个接口，意味着又多扮演了一个角色。多了一个身份,多了一份能力:多了一个身份时对外来说的,因为继承了一个接口,那么这个类在在作为入参是就多了一个选择.
     多了一份能力:一个类实现了一个接口之后,那么它就要实现这个接口中的方法,功能上更加强大.
     ...
     设计原则：
        1.高层角色在做一件事情的时候，如果向交给下层模块去做的话，那么为了达到内聚的特性，尽量完全由这个底册模块完成，高层不要持有这个底层模块的返回值，
